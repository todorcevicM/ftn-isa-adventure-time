			...
			<div class="leftFlex">
				...
				<div v-if="canUpload">
					...
					<input
						...
						@change="onFileChange"
					/>
					<button @click="uploadImage(adventure.id)">
						Upload
					</button>
...
export default {
	setup() {
		...
		axios.post("/api/image/existsByIdAndType/" + adventure.value.id, "ADVENTURE").then(function (response) {
		...
		return {
			...
			updateDetails() {
				axios
					.get("/api/adventures/checkAvailable/" + adventure.value.id)
					.then(function (response) {
						if (!response.data) {
							alert(
								"This adventure is not available for editing as a booking currently exists."
							);
						} else {
